...
class FreePractice:
    ...
    def __init__(self, racetrack: RaceTrack, mpc: "NMPC", perturbe_parameters: dict = True):
        ...
        if not self.mpc.are_references_set():
            raise ValueError("References are not set in the MPC. Please set the references before starting the free practice session.")
        ...
        if perturbe_parameters:
            ...
            self.mpc.model.parameters.air_density *= np.random.uniform(0.8, 1.2)
    ...
    def animated_lap(self, s_0: float):
        ...
        prediction_line, = ax_track.plot([], [], 'r-', linewidth=2, label="Prediction")
        ...
        def update(frame):
            ...
            car_dot.set_data(
                [self.racetrack.position(current_s)[0] + current_x[N_INDEX] * self.racetrack.normal_vector(current_s)[0]],
                [self.racetrack.position(current_s)[1] + current_x[N_INDEX] * self.racetrack.normal_vector(current_s)[1]]
            )
            ...
            lap_time   += dt
            current_x = self.mpc.model.step_dt(current_x, control_input, kappa=current_kappa, dt=dt).full().flatten()

            if current_s >= self.racetrack.length:
                ani.event_source.stop()
                print("Lap completed!")
                print("Total lap time:", lap_time, "seconds")
                print("Average cpu time per mpc call:", np.mean(mpc_time), "seconds")
                ...
                ax_cmp.plot(xy_ref_np[:,0],    xy_ref_np[:,1],    '--', linewidth=2, label='Raceline')
                ax_cmp.plot(xy_driven_np[:,0], xy_driven_np[:,1], '-',  linewidth=2, label='Driven')
        ...
        # Animate
        # -------------------
        ani = FuncAnimation(fig_track, update, frames=num_frames, interval=30, blit=True)
        plt.show()
    ...
    def fast_lap(self, s_0: float):
        ...
        for frame in range(num_frames):
            ...
            print("Percentage progress along the track:", (current_s/self.racetrack.length)*100)
            print("state error norm:", np.linalg.norm(current_x - reference_state))
...
class GrandPrix:
    ...
    def animated_race(self, s0_1: float, s0_2: float):
        ...
        pred1_line, = ax_track.plot([], [], 'r-', linewidth=2, label="Prediction 1")
        pred2_line, = ax_track.plot([], [], 'b-', linewidth=2, label="Prediction 2")

        # -------------------
        # States figure
        # -------------------
        fig_states, axes_states = plt.subplots(n_states, 1, figsize=(6, 2*n_states), sharex=True)
        state_lines1 = [axes_states[i].plot([], [], 'r', label=f"x{i} car1")[0] for i in range(n_states)]
        state_lines2 = [axes_states[i].plot([], [], 'b', label=f"x{i} car2")[0] for i in range(n_states)]
        state_names = [r'$\psi$', r'$v_x$', r'$v_y$', r'$\dot{\psi}$', r'$\delta$', r'$s$', r'$\xi$']
        ...
        # Inputs figure
        # -------------------
        fig_inputs, axes_inputs = plt.subplots(n_inputs, 1, figsize=(6, 2*n_inputs), sharex=True)
        input_lines1 = [axes_inputs[i].plot([], [], 'r--', label=f"u{i} car1")[0] for i in range(n_inputs)]
        input_lines2 = [axes_inputs[i].plot([], [], 'b--', label=f"u{i} car2")[0] for i in range(n_inputs)]
        ...
        def update(frame):
            ...
            for i in range(n_states):
                state_lines1[i].set_data(s_values1[:frame+1], states1[:frame+1, i])
                state_lines2[i].set_data(s_values2[:frame+1], states2[:frame+1, i])
                axes_states[i].relim()
            ...
            for i in range(n_inputs):
                input_lines1[i].set_data(s_values1[:frame+1], inputs1[:frame+1, i])
                input_lines2[i].set_data(s_values2[:frame+1], inputs2[:frame+1, i])
                axes_inputs[i].relim()
            ...
            x1 = self.mpc1.model.step_dt(x1, u1, kappa=kappa1, dt=dt).full().flatten()
            x2 = self.mpc2.model.step_dt(x2, u2, kappa=kappa2, dt=dt).full().flatten()
        ...
        # Animate
        # -------------------
        ani = FuncAnimation(fig_track, update, frames=num_frames, interval=10, blit=True)
        plt.show()
